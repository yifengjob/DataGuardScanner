...
ContentInspector 是一款基于 Tauri 2.x 和 Vue 3 构建的跨平台桌面应用程序，专门用于扫描和检测文件系统中的敏感数据。它能够智能识别身份证号、手机号、邮箱、银行卡号、地址、IP 地址和密码等隐私信息，并提供可视化的高亮预览和报告导出功能。

### 核心优势

- 🔍 **智能检测**：采用正则表达式 + 校验算法（Luhn、身份证校验码）确保准确性
- ⚡ **高性能**：基于 Rust 后端，支持并发扫描，处理大文件高效稳定
- 🎯 **多格式支持**：支持文本文件、PDF、Excel、Word、PowerPoint、RTF 等多种格式
- 🌐 **跨平台**：完美支持 Windows、macOS 和 Linux
- 📊 **可视化报告**：支持 CSV、JSON、Excel 三种格式导出扫描结果
- 🔒 **安全可靠**：本地运行，数据不上传，保护隐私安全
- 🚀 **流式处理**：支持大文件流式读取和预览，内存占用低
...
- Word 文档（.docx 流式解析，.doc 实验性支持）
- PowerPoint 演示文稿（.pptx 流式解析，.ppt 实验性支持）
- RTF 文档（流式解析）
...
- ⚠️ **不支持 Windows 7/8/8.1**（需要较新的 WebView2 运行时）
...
#### Linux
- **x86_64 (Intel/AMD)**：Ubuntu 20.04+、Debian 11+、Fedora 35+
- **ARM64 (aarch64)**：Ubuntu 20.04+ ARM、Raspberry Pi OS 64-bit、其他 ARM64 Linux 发行版
- WebKit2GTK 4.1+
- GTK 3+
- libsoup 3+
...
2. 安装 [Node.js](https://nodejs.org/)（24+ 推荐，最低 22+）
...
│   │   │   └── preview-virtual-scroller.ts  # 虚拟滚动
│   │   ├── App.vue          # 主应用组件
│   │   └── main.ts          # 入口文件
│   ├── package.json
│   └── vite.config.ts
...
│   │   │       └── office/          # Office 文档解析
│   │   ├── processing/      # 处理模块
│   │   │   ├── file_stream_processor.rs  # 流式处理
│   │   │   └── preview.rs           # 预览功能
│   │   └── utils/           # 工具模块
│   │       ├── config.rs            # 配置管理
│   │       ├── logger.rs            # 日志系统
│   │       ├── cache_cleanup.rs     # 缓存清理
│   │       ├── environment_check.rs # 环境检查
│   │       ├── excel_export.rs      # Excel 导出
│   │       ├── expression_parser.rs # 表达式解析
│   │       ├── file_types.rs        # 文件类型注册
│   │       ├── concurrency.rs       # 并发控制
│   │       └── ...                  # 其他工具函数
│   ├── build.rs           # 构建脚本
│   ├── Cargo.toml
...
│   └── compare_projects.sh  # 项目对比脚本
│
├── docs/                    # 技术文档
│   ├── THEME_GUIDE.md
│   ├── PDF_ERROR_HANDLING.md
│   └── ...                  # 其他开发文档
│
...
### v1.0.5 (当前版本 - 2026-05-19)

#### ✨ 架构优化
- **配置管理统一**：使用 build.rs 从 tauri.conf.json 读取 identifier
  - 所有模块使用统一的 APP_IDENTIFIER 常量
  - 移除硬编码的应用标识符
  - Serde 自动序列化转换（rename_all = "camelCase"）
- **前端代码简化**：删除手动 camelCase/snake_case 转换逻辑
  - saveConfig 和 startScan 函数简化
  - 减少 34 行冗余代码
  - 提高可维护性

#### 🐛 Bug 修复
- **文件预览修复**：修正 get_processor_type 调用方式
  - 正确提取文件扩展名而非传入完整路径
  - 解决非文本文件无法预览的问题
  - 支持 docx、xlsx、pdf 等格式预览

#### 🧹 功能优化
- **缓存清理重构**：移除无用的 WebView 缓存清理
  - Tauri 使用系统 WebView，缓存由系统管理
  - 专注于清理应用日志和临时文件
  - 更新 UI 提示文本，更清晰明确
- **Electron 遗留代码清理**：
  - 删除 electron-api.ts 文件
  - 标记废弃函数并添加警告提示
  - 修复开发者工具调用

#### 🔧 技术改进
- **严格模式**：环境变量配置无默认值，编译时强制检查
  - 确保配置一致性
  - 及早发现问题
  - 防止隐蔽错误
- **代码组织优化**：
...
#### 🛡️ 稳定性增强
- **单文件超时保护**：防止单个文件卡住整个扫描
  - 动态超时计算（基于文件大小和类型）
  - 优雅超时处理
  - 详细的超时日志
- **停滞检测优化**：缩短检测间隔，提高响应速度
  - 15秒警告阈值
  - 120秒强制停止
  - 自动恢复机制
...
#### 🚀 CI/CD 改进
- **Linux ARM64 支持**：完整支持 aarch64 架构交叉编译
  - QEMU 模拟环境配置
  - Ubuntu Ports 仓库集成（ports.ubuntu.com）
  - 多架构 APT 源分离配置（archive + ports）
  - 依赖冲突解决（移除 libgirepository1.0-dev）
  - AppImage 打包跳过策略（linuxdeploy 兼容性问题）
  - 产物格式：.deb + .rpm（跳过 .AppImage）
- **Node.js 24 迁移**：提前适配 GitHub Actions 强制迁移
  - actions/checkout: v4 → v6
  - pnpm/action-setup: v4 → v5 ⭐（专门支持 Node.js 24）
  - actions/setup-node: v4 → v6
  - actions/upload-artifact: v4 → v7
  - actions/download-artifact: v8
  - 消除 "Node.js 20 deprecated" 警告
- **安全性提升**：所有 APT 源从 HTTP 升级为 HTTPS
- **产物完整性**：修复 RPM 包上传路径遗漏问题
- **Artifact 管理**：统一使用 if-no-files-found: ignore 处理平台差异
...
本项目采用 AGPL-3.0 license 许可证 - 详见 [LICENSE](LICENSE) 文件
